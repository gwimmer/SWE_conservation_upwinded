...
upwinding for u only, and Poisson-type time discretisation"""
...
if COMM_WORLD.Get_rank() == 0:
    print("Simulation for Williamson5 test case, model u-ad_flux", "\n",
...
u_ad = (inner(perp(grad(inner(Dbar*w, perp(u_rec)))), u_bar)*dx
        + inner(jump(inner(Dbar*w, perp(u_rec)), n),
                p_uw(ubar, u_bar))*dS - f*inner(perp(u_rec), Dbar*w)*dx)
...
# Auxiliary solvers
Feqn = inner(w, u_ - Frhs)*dx
Fproblem = LinearVariationalProblem(lhs(Feqn), rhs(Feqn), F)
Fsolver = LinearVariationalSolver(Fproblem, solver_parameters=
                                  {"ksp_type":"preonly",
                                   "pc_type":"lu"})
